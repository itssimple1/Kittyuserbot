...
async def admin_groups(kitty):
    kittygroups = []
    async for dialog in kitty.client.iter_dialogs():
...
async def make_gif(event, file):
    ...
    async with event.client.conversation(chat) as conv:
        try:
            ...
            kittyresponse = response if response.media else await conv.get_response()
            await event.client.send_read_acknowledge(conv.chat_id)
            kittyfile = await event.client.download_media(kittyresponse, "./temp")
            return await unzip(kittyfile)
...
async def iphonex(text):
    ...
    its_simple1 = r.get("message")
    kittyurl = url(its_simple1)
    if not kittyurl:
        return "check syntax once more"
    with open("temp.png", "wb") as f:
        f.write(requests.get(its_simple1).content)
...
async def waifutxt(text, chat_id, reply_to_id, bot, borg):
    ...
    kitty = await sticcers[0].click("me", hide_via=True)
    if kitty:
        await bot.send_file(int(chat_id), kitty, reply_to=reply_to_id)
        await kitty.delete()
