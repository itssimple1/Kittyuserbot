...
def load_module(shortname):
    if shortname.startswith("__"):
        ...
        LOGS.info("KittyUserBot - Successfully imported " + shortname)
    else:
        import userbot.utils
        ...
        mod.Config = Config
        mod.borg = bot
        mod.kittyuserbot = bot
        mod.edit_or_reply = edit_or_reply
        mod.delete_kitty = delete_kitty
        mod.media_type = media_type
        # support for kittyuserbot originals
        sys.modules["kittyuserbot.utils"] = userbot.utils
        sys.modules["kittyuserbot"] = userbot
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.COMMAND_HAND_LER) == 2:
                kittyreg = "^" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER[1]
            elif len(Config.COMMAND_HAND_LER) == 1:
                kittyreg = "^\\" + Config.COMMAND_HAND_LER
                reg = Config.COMMAND_HAND_LER
            args["pattern"] = re.compile(kittyreg + pattern)
    ...
    if pattern is not None:
        if pattern.startswith(r"\#"):
            ...
            if len(Config.SUDO_COMMAND_HAND_LER) == 2:
                kittyreg = "^" + Config.SUDO_COMMAND_HAND_LER
...
async def delete_kitty(event, text, time=None, parse_mode=None, link_preview=None):
    parse_mode = parse_mode or "md"
    link_preview = link_preview or False
    time = time or 5
    if event.sender_id in Config.SUDO_USERS:
        ...
        kittyevent = await event.edit(
            text, link_preview=link_preview, parse_mode=parse_mode
        )
    await asyncio.sleep(time)
    return await kittyevent.delete()
...
async def unsavegif(event, its_simple1):
    try:
        await event.client(
            functions.messages.SaveGifRequest(
                id=types.InputDocument(
                    id=its_simple1.media.document.id,
                    access_hash=its_simple1.media.document.access_hash,
                    file_reference=its_simple1.media.document.file_reference,
                ),
                unsave=True,
            )
        )
