...
class KittyBroadcast(BASE):
    __tablename__ = "kittybroadcast"
    ...
    def __repr__(self):
        return "<Kitty Broadcast channels '%s' for %s>" % (self.group_id, self.keywoard)

    def __eq__(self, other):
        return bool(
            isinstance(other, KittyBroadcast)
            and self.keywoard == other.keywoard
            and self.group_id == other.group_id
        )


KittyBroadcast.__table__.create(checkfirst=True)

KittyBROADCAST_INSERTION_LOCK = threading.RLock()
...
def add_to_broadcastlist(keywoard, group_id):
    with KittyBROADCAST_INSERTION_LOCK:
        broadcast_group = KittyBroadcast(keywoard, str(group_id))
...
def rm_from_broadcastlist(keywoard, group_id):
    with KittyBROADCAST_INSERTION_LOCK:
        broadcast_group = SESSION.query(KittyBroadcast).get((keywoard, str(group_id)))
        if broadcast_group:
...
def del_keyword_broadcastlist(keywoard):
    with KittyBROADCAST_INSERTION_LOCK:
        broadcast_group = (
            SESSION.query(KittyBroadcast.keywoard)
            .filter(KittyBroadcast.keywoard == keywoard)
            .delete()
        )
...
def get_broadcastlist_chats():
    try:
        chats = SESSION.query(KittyBroadcast.keywoard).distinct().all()
...
def num_broadcastlist():
    try:
        return SESSION.query(KittyBroadcast).count()
    finally:
        SESSION.close()
...
def num_broadcastlist_chats():
    try:
        return SESSION.query(func.count(distinct(KittyBroadcast.keywoard))).scalar()
    finally:
        SESSION.close()
...
def __load_chat_broadcastlists():
    try:
        ...
        all_groups = SESSION.query(KittyBroadcast).all()
        for x in all_groups:
